...
const camposNumericos = [
  ...
  { key: "cintura", label: "Circunferência da cintura", unit: "cm" },
  { key: "quadril", label: "Circunferência do quadril", unit: "cm" },
  { key: "abdomen", label: "Circunferência abdominal", unit: "cm" },
  { key: "braco", label: "Circunferência do braço", unit: "cm" },
];
...
export default function Antropometria() {
  ...
  const rcq = cintura && quadril ? (cintura / quadril).toFixed(2) : "";

  async function salvar() {
    if (!firebaseConfigured) return;
    setSalvando(true);
    try {
      await criarDocumento(`${pacientePath}/antropometria`, {
        ...form,
        imc: imc ? Number(imc) : null,
        rcq: rcq ? Number(rcq) : null,
        atendimentoId,
        profissionalId,
        ativo: true,
      });
  ...
  const chartData = historico
    .filter((h) => h.imc)
    .map((h) => ({ date: h.criadoEm?.toDate ? h.criadoEm.toDate().toLocaleDateString("pt-BR", { day: "2-digit", month: "short" }) : "", imc: h.imc }));
      ...
      <div className="space-y-3">
        <div className="card p-4 grid grid-cols-2 gap-x-4 gap-y-4">
          ...
          <div>
            ...
            <div className="text-[10px] text-ink-500 mt-0.5">kg/m² (calculado)</div>
          </div>
          <div>
            <label className="text-xs font-medium text-ink-900 mb-1 block">RCQ:</label>
            <input readOnly value={rcq} className="w-full text-sm border border-black/10 rounded-lg px-2.5 py-1.5 bg-brand-50/60 text-brand-700 font-semibold focus-ring" />
            <div className="text-[10px] text-ink-500 mt-0.5">relação cintura-quadril (calculado)</div>
          </div>
        </div>
        <button onClick={salvar} disabled={salvando || !firebaseConfigured} className="w-full flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white text-sm font-semibold py-2.5 rounded-lg focus-ring">
          {salvando ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Registrar medidas
        </button>
      </div>
